...
session_start();
//echo $_POST['catcta'];
//echo $_SESSION['catcta'];
//echo $_SESSION['catcta1'];

if (isset ($_REQUEST['post_type']) && 'register' == $_REQUEST['post_type'] && $_REQUEST['catcta'] != $_SESSION['catcta']) {
    //    wp_die( "Please input correct catcta!" );
    wp_redirect("/conference-registration?catcta=1");
    exit;
}

if (isset ($_REQUEST['post_type']) && 'register' == $_REQUEST['post_type'] && $_REQUEST['company'] == 'google') {
    //    wp_die( "Please input correct catcta!" );
    wp_redirect("/conference-registration");
    exit;
}

$add_ip_meta = false;
if (isset ($_REQUEST['post_type']) && 'register' == $_REQUEST['post_type']) {
    //    add_post_meta( $post_ID, 'register_ip', $_SERVER['REMOTE_ADDR'] );
    $add_ip_meta = true;
}
...
if ($add_ip_meta) {
    add_metadata('post', intval($post_id), 'regiser_ip', $_SERVER['REMOTE_ADDR']);
}
